...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//GET방식으로 요청시 해당 doGET메서드가 저절로 호출된다.(톰캣이 서블릿객체를 생성해서 메서드 호출까지 함)
		System.out.println("GET요청 도착");
		
		/*
		 * 첫번쨰 매개변수인 request에는 요청시 전달된 내용들이 담겨있음(사용자가 입력한 값, 요청방식, 요청자의 url등...)
		 * 두번쨰 매개변수인 request에는 요청 처리 후 응답에 사용되는 객체(어떤타입으로 응답을 할지, 어떤 값을 응답할지등을 넣어주면 됨)
		 * 
		 * 요청처리를 위해서 요청시 값을 추출
		 * request의 paramter영역안에 존재
		...
		//즉, 여기 java코드내에 사용자가 보게될 응답 html구문을 작석
