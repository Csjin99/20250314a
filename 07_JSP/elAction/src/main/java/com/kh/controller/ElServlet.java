...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 
		 * 1.SerlvetContext(applicaion scope)
		 ...
		 * 한 브라우저당 1개 존재하는 객체
		 * 이 영역에 데이터를 담으면 jsp/servlet단에서 사용이 가능하다
		 * 
		 * 3.HttpServletRequest(request scope)
		 * 요청때마다 매번 생성되는 객체
		 * 이 영역에 데이터를 담으면 해당 request객체를 포워딩받는 응담jsp에서만 사용 가능
		 * 
		 * 4. PageContext(page scope)
		 * jsp마다 존재하는 객체
		 * 공유범유ㅣ가 가장 좁다.(해당 페이지에서만 공유)
		 * 
		 * 위 4개의 객체들에
		 * 데이터를 담을 때 -> .setAttribute("키",값);
		 * 데이터를 꺼낼 때 -> .getArrtibute("키");
		 * 데이터를 삭제할 때 -> .removeArrtibute("키");
		 */
		
		//application scope
		ServletContext apllication = request.getServletContext();
		apllication.setAttribute("scope", "apllication");
