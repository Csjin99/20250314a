...
public class BoardController {
    ...
    @GetMapping("piclist.bo")
    public String selectPicBoardList(@RequestParam(defaultValue = "1") int cpage, Model model) {
        int boardCount = boardService.selectPicBoardCount();

        PageInfo pi = new PageInfo(boardCount, cpage, 10, 5);
        ArrayList<PicBoard> list = boardService.selectPicBoardList(pi);
        System.out.println(list.get(0).getChangeName());
        model.addAttribute("list", list);
        model.addAttribute("pi", pi);
        return "board/picBoardListView";
    }

    @GetMapping("picEnrollForm.bo")
    public String picEnrollForm(){
        return "board/picboardEnrollForm";
    }

    @PostMapping("picinsert.bo")
    public String picinsert(@ModelAttribute PicBoard picBoard, MultipartFile upfile, HttpSession session, Model model) {

        if(!upfile.getOriginalFilename().equals("")){
            String changeName = Template.saveFile(upfile, session, "/resources/uploadfile/");

            picBoard.setChangeName("/resources/uploadfile/" + changeName);
            picBoard.setOriginName(upfile.getOriginalFilename());
        }

        int result = boardService.picinsertBoard(picBoard);

        if(result > 0){
            session.setAttribute("alertMsg", "게시글 작성 성공");
            return "redirect:/piclist.bo";
        } else {
            model.addAttribute("errorMsg", "게시글 작성 실패");
            return "common/errorPage";
        }
