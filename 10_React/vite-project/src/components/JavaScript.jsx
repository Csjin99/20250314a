...
const imgUrl =
  'https://i.namu.wiki/i/uwKtyJQdxUUgL4cU3NiJmWGeC7fm1ezwU2u5MebKQ9hw6tXWQzIzUG0CDmDtKjyIqhtHvnIZKNabLNztdJwiIQ.webp';
  ...
  return (
    <div>
      ...
      <ul>
        {userList.map((user) => (
          <li key={user.id}>
            이름: {user.name}, id: {user.id}, age: {user.age}
          </li>
        ))}
      </ul>
      {/* 동적으로 ui생성시 react가 변경 항목만을 식별하기위해 key속성이 필요함(key가 없으면 변경됭 항목을 잘못 판단할 수있음) */}
    </div>
  );
};
