...
/*
    ...
    *트랜잭션
    ...
    DML문 한개를 수행할 때 트랜잭션이 존재하지않는다면 트랜잭션을 만들어서 묶음.
    ...
    COMMIT : 한트랜잭션에 담겨있는 변경사항들을 실제 DB에 반영하겠다.
...
--오토커밋 수동설정 / ON->자동(DML문 실행마다 무조건 COMMIT
SET AUTOCOMMIT OFF;
...
INSERT INTO EMP_01
VALUES(801,'김가네','기술지원부');

INSERT INTO EMP_01
VALUES(802,'구가네','창업지원부');
...
--즉, DLL문 수행전 변경사항들이 있다면 정확하게 픽스하고 해라!


/*
    ACID속성: 트랜잭션의 일관성과 실뢰성을 보장하기위해 지켜야하는 네가지 핵심속성
    원자성: 트랜잭션 내의 작업이 모두 완료(COMMIT)되거나, 모두 취소(ROLLBACK)되어야함.
    일관성: 트랜잭션이 실행되기 전과 후에 데이터베이스의 상태가 항상 일관성을 유지해야한다
