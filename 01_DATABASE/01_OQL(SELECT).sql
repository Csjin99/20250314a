...
--EMPLOYEE 테이블의 사원명, 이메일, 전화번호, 일사일, 급여조회
...
-- <컬럼값을 통한 산술연산>
-- SELCET절 컬럼명 작성부분에 산술연산을 할 수 있다.

--EMPLOYEE테이블의 사원명, 사원의 연봉(SALARY * 12)를 조회
SELECT EMP_NAME, SALARY * 12
FROM EMPLOYEE;

--EMPLOYEE테이블의 사원명, 급여, 보너스, 사원의 보너스 포함연봉((급여+(급여*보너스)) *12)를 조회

SELECT EMP_NAME, SALARY, BONUS, ((SALARY + (SALARY * BONUS)) * 12) FROM EMPLOYEE;

--데이터베이스에서 NULL은 빈값을 의미한다
--모든 연산자에 NULL이 포함되는경우 결과는 NULL이다.

--사원명, 입사일, 근무일수를 조회
--데이터베이스에서는 날짜를 계산할때 덧셈 뺄셈이 가능하다.
--현재시간- 입사일 =근무한시간
--DATE- DATE=> 결과는 무조건 일로 표시된다.
-- 코드실행시 현재날짜를 표시하는 상수 : SYSDATE(년/월/일/시/분/초)
...
--SELECT DISTINCT JOM_CODE, DITINCT DEPT_CODE
--위처럼 작성시 에러가 발생한다. 한명령어에 DISTINCT는 한번만 사용가능
...
SELECT EMP_NAME, SALARY, HIRE_DATE, SALARY * 12 AS "연봉" FROM EMPLOYEE
WHERE SALARY >= 3000000;


--2. 연봉이 5천만원 이상인 사람들의 사원명 급여 연봉(별칭 -> 연봉) 부서코드 조회
SELECT EMP_NAME, SALARY, SALARY * 12 AS "연봉",DEPT_CODE FROM EMPLOYEE
...
--4. 급여가 350이상 600만원 이하인 모든 사람원의 사원명 사번 급여 조회
...
/*
    ...
    조건을 여러개 연결할 때 사용한다.
    [표현법]
    조건1 AND 조건2 -> 조건1 과 조건2 둘다 만족하는 값만 참으로 한다
    조건1 OR 조건2 -> 둘중하나라도 만족하면 참으로 한다.
...
-- NULL을 비교할 때는 IS NULL나 IN NOT NULL을 사용한다.
SELECT * FROM EMPLOYEE
--WHERE BONUS IS NULL;
...
/*
    <LIKE>
    비교하자는 컬럼값이 내가 제시한 특정 패턴에 만족할 경우 조회
    ...
    EX) 비교할 대상 컬럼 LIKE '문자%': 비교할대상컬럼값 중에서 해당 문자로 시작하는 값들을 전부 가져온다.
        비교할 대상 컬럼 LIKE '%문자': 비교할대상컬럼값 중에서 해당 문자로 끝나는 값들을 전부 가져온다.
        비교할 대상 컬럼 LIKE '%문자%': 비교할대상컬럼값 중에서 해당 문자가 포함된 값은 전부 가져온다.
    
    2.'_': 1글자를 대체검색할때 사용
    EX) 비교할 대상 컬럼 LIKE '_문자': 비교할대상 컬럼값 문자앞에 아무글자나 딱 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '문자_': 비교할대상 컬럼값 문자뒤에 아무글자나 딱 한글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '_문자_': 비교할대상 컬럼값 문자앞,뒤에 아무글자나 딱 한글자가 있는 값을 조회
        _하나당 글자 하나(문자수 조절가능)
...
--사원들 중에서 전화번호 세번째 자리가 1인사원들의 사번, 사원명, 전화번호 조회
...
FROM EMPLOYEE
--WHERE EMAIL LIKE '___%'; ->와일드카드로 인식되기 때문에 정상적으로 출력할수 없다.
--와일드카드와 일반문자를 구분해줘야한다.
--데이터값으로 취급하고싶은 와일드카드 문자 앞에 나만의 탈출문자를 제시해서 탈출시켜준다.
...
--4. 부서테이블에서 해외영업부인 부서들의  코드, 부서명 조회
SELECT DEPT_ID, DEPT_TITLE
FROM department
WHERE DEPT_TITLE LIKE '%해외영업%';
--5. 사수가 없고 부서배치도 받지않은 사원들의 사원명, 사번, 부서코드 조회
SELECT EMP_NAME, EMP_ID, DEPT_CODE
FROM EMPLOYEE
WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;
--6. 연봉(급여 * 12)이 3천만원 이상이고 보너스를 받지않는사원들의 사번, 사원명, 급여, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, BONUS
FROM EMPLOYEE
WHERE SALARY * 12 >= 30000000 AND BONUS IS NULL;
--7. 입사일이 95/01/01이상이고 부서배치를 받지않는 사원들의 사번, 사원명, 입사일, 부서코드 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE, DEPT_CODE
FROM EMPLOYEE
WHERE HIRE_DATE >= '95/01/01' AND DEPT_CODE IS NULL;
--8. 급여가 200만원 이상이고 500만원 이하인 사원 중에서 입사일이 01/01/01 이상이고 보너스를 받지않은
--   사원들의 사번, 사원명, 급여, 입사일, 보너스 조회
SELECT EMP_ID, EMP_NAME, SALARY, HIRE_DATE, BONUS
FROM EMPLOYEE
WHERE salary BETWEEN  2000000 AND 5000000 AND HIRE_DATE >= '01/01/01' AND BONUS IS NULL;
--9. 보너스를 포함 연봉이 NULL이 아니고 이름에 '하'가 포함된 사원들의 사번, 사원명, 급여, 보너스 포함연봉 조회
SELECT EMP_ID, EMP_NAME, SALARY, ((SALARY + (SALARY * BONUS)) *12) AS "보너스 포함연봉"
FROM EMPLOYEE
