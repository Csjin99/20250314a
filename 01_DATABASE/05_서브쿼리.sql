...
--서브쿼리 예시1) 노옹철 사원과 같은 부서에 속한 사원들ㅇ 전체 조회
...
--서브쿼리 예시2)
--전직원의 평균 급여보다 더 많은 급여를 받는 사원들의 사번 이름 직급코드 급여 조회
--전 직원의 평균 급여
...
/*
    *서브쿼리의 구분
    ...
    --다중행 다중열 서브쿼리 : 서브쿼리의 조회 결과값이 여러행 여러컬럼일떄
...
/*
    ...
    1.단일행 서브쿼리
    서브쿼리 결과로 값이 1개일때 사용한다(한행 한열)
    일반적으로 비교연산자 사용가능
    = != < > <= .....

*/

--1) 전 직원의 평균급여보다 급여를 더 적게받는 사원들의 사원명, 직급코드, 급여조회
...
/*
    ...
    비교대상 > ANY (서브쿼리): 여러개의 결과값 중에서 한개라도 클경우 조회
...
--1) 유재석 또는 윤은해사원과 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
---- 다중열 서브쿼리
--튜플비교 : 두개 이상의 열(컬럼)을 조합해서 비교한느 방식
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
/*
    4. 다중행 다중열 서브쿼리
    서브쿼리의 조회 결과값이 여러행 여러열일 경우 (주로 튜플비교와 함께 사용된다.)
...
--각 부서별 최고급여를 바든 사원들의 사번 사언명 부서코드 급여
...
--각 부서별 급여 합계가 전체 급여 총 합의 20%보다 많은 부서의 부서면, 부서별 급여합계 조회
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
-- 전 직원중 급여가 가장 높은 5명만 조화ㅣ
--ROWNUM: 오라클에서 제공해주는 컬럼, 조회된 순서대로 1부터 순번을 부여해주는 컬럼
--  ROWNUM은 FROM절에서 결정된다
SELECT ROWNUM , EMP_NAME, SALARY
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;

-->ORDER BY절이 수행된 결과를 가지고 ROWNUM부여_ 상위5명
SELECT ROWNUM, EMP_NAME, SALARY
...
--인라인뷰에 있는 산술식으로된 컬럼을 가져오기 위해서는 별칭을 부여해야한다.
SELECT SAL_AVG
FROM(SELECT DEPT_CODE, FLOOR(AVG(SALARY)) AS "SAL_AVG"
    FROM EMPLOYEE
    GROUP BY DEPT_CODE
