/*
    <ORDER BY절>
    SELECT문 가장 마지막 줄에 작성, 상행순서또한 가장 마지막에 실핸한다.
    
    [표현법]
    SELECT 조회할 컬럼
    FROM 조회할 테이블
    WHERE 조건식
    ORDER BY 정렬기준이 될 컬럼명 | 별칭 | 컬럼순번 [ASC | DESC ] [ NULLS FIRST | NULLS LAST] ([]안에 있는건 생략가능)
    ...
    NULLS FIRST: 정렬하고자하는 컬럼값에 NULL이 있을경우 해당 데이터 맨 앞에 배치(DESC일 때 기본값)
    NULLS LAST: 정렬하고하는 컬럼값에 NULL이 있을 경우 해당 데이터 맨 뒤에 배치(ASC일 때 기본값)
...
/*
    ...
    SELECT 절에 단일행 함수와 그룹함수는 같이 사용 못한다.
...
SELECT INSTR('AAAAABBBBBCCCCCDDDAA', 'B') FROM DUAL;
--찾을 위치 시작값 : 1, 순번 : 1 -> 기본값
SELECT INSTR('AAAAABBBBBCCCCCDDDAA', 'B', 1) FROM DUAL;
SELECT INSTR('AAAAABBBBBCCCCCDDDAA', 'B', -1) FROM DUAL; -- 뒤에서부터 찾지만 읽을 떄는 원래대로 읽어준다.
SELECT INSTR('AAAAABBBBBCCCCCDDDAA', 'B', 1,2) FROM DUAL;
SELECT INSTR('AAAAABBBBBCCCCCDDDAA', 'B', 1,3) FROM DUAL;
...
--사원들의 사원명, 주민등록번호("123456-1XXXXXXXX")
...
SELECT LTRIM('         S        G   ') FROM DUAL; --앞에서부터 다른 문자가 나올때까지 공백 제거

SELECT LTRIM('ACBABACKH', 'ABC') FROM DUAL;


/*
    *TRIM
    문자열의 앞/뒤/양쪽에있는 저장한 문자들을 제거한 나머지 무자열 반환
    TRIM([LEADING| TRAILING | BOTH] 제고하고자하는 문자열 FROM 문자열)
    

*/
SELECT TRIM('         S        G   ') FROM DUAL;
SELECT TRIM(BOTH 'Z' FROM'ZZZZZZZZZZSZZZZZZZZZZ') FROM DUAL;
SELECT TRIM(LEADING 'Z' FROM'ZZZZZZZZZZSZZZZZZZZZZ') FROM DUAL;
SELECT TRIM(TRAILING 'Z' FROM'ZZZZZZZZZZSZZZZZZZZZZ') FROM DUAL;

/*
    *LOWER / UPPER / INITCAP
    LOWER: 다 소문자로 변경항 문자열 반환
    UPPER: 다 대문자로 변경한 문자열 반횐
...
*/
SELECT ROUND(15.2355) FROM DUAL; -- 기본자수는 소수점 첫번쨰 자리에서 반올림
SELECT ROUND(15.2355, 3) FROM DUAL; -- 양수로 증가할수록 소수점 뒤로 한칸씩 이동
SELECT ROUND(15.2355, -1) FROM DUAL; -- 음수로 감소할수록 소수점 앞자리로 이동
...
SELECT CEIL(13.564) FROM DUAL; -- 올림자리를 지정못함
...
SELECT TRUNC(12345.12356, 3) FROM DUAL;
SELECT FLOOR(12345.12356) FROM DUAL;
...
--사원들의 사원명, 입사일, 근무일수, 근무개월수를 조회하라
SELECT EMP_NAME, HIRE_DATE, FLOOR(SYSDATE- HIRE_DATE) AS "근무일수" , CEIL( MONTHS_BETWEEN(SYSDATE , HIRE_DATE)) AS "근무개월수"
FROM EMPLOYEE; 
...
-- 시스템 언어 변경
ALTER SESSION SET NLS_LANGUAGE = AMARICAN;
...
/*
    *EXTRACT : 특정 날짜로 부터 년|월|일 값을 추출해서 반환하는 함수
    [표현법]
...
SELECT TO_CHAR(13, '99999') FROM DUAL; -- 9의 자리수만큼 공간 확보, 빈칸을 공백으로 채움
SELECT TO_CHAR(13, '00000') FROM DUAL; -- 0의 자리수만큼 공간 확보, 빈칸을 0으로 채움
SELECT TO_CHAR(200000, 'L9999999') FROM DUAL; --현재 설정된 나라의 로컬 화폐단위 나타냄

SELECT TO_CHAR(200000, '99,999,99') FROM DUAL;
...
SELECT TO_CHAR(SYSDATE, 'PM HH:MI:SS') FROM DUAL; -- AM. PM 어떤것을 사용하건 형식은 동일하다
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
       TO_CHAR(SYSDATE, 'RRRR'), --RR룰이 따로 존재한다. -> 50년이상이면 다음 세기, 50년이하면
       TO_CHAR(SYSDATE, 'YY'),
       TO_CHAR(SYSDATE, 'YEAR') FROM DUAL;


SELECT
 TO_CHAR(TO_DATE('24', 'RRRR'), 'RRRR'), -- 현재세기(2000)으로 해석
...
--일에 관련된 포맷
SELECT TO_CHAR(SYSDATE, 'DDD'), -- 오늘이 이번년도 몇번쨰 일수
...
SELECT TO_DATE(601218) FROM DUAL; --> 2자리만 입력시 기본적으로 RR룰을 적용한다

SELECT TO_DATE(051010) FROM DUAL; -- 0으로 시작하는 숫자는 없다.
SELECT TO_DATE('051010') FROM DUAL;
SELECT TO_DATE('051010 120550', 'YYMMDD HH24MISS') FROM DUAL; -- 포맷을 정해주게 되어있음
...
SELECT TO_NUMBER('46645468465164165') FROM DUAL;

SELECT '1000' + '3000' FROM DUAL; --문자열 -> 숫자로 자동 변환이 지행된다.
SELECT '1000,000' + '3000' FROM DUAL;
SELECT TO_NUMBER('1000,000', '9999,999') + TO_NUMBER('3000,000', '9999,999') FROM DUAL;
...
SELECT EMP_NAME, (SALARY + (SALARY* NVL(BONUS, 0)) *12) AS "포함연봉"
FROM EMPLOYEE;
...
--* NULLIF(비교대상1, 비교대상2)
--두값이 일치하면 NULL이고 일치하지않으면 비교대상 1을 반환
...
--2. AVG : 해당 컬럼값들의 평균을 구해서 반환
SELECT AVG(SALARY)
FROM EMPLOYEE;

--3. MIN(모든타임의 가능) : 해당 컬럼값 중 가장 작은 값을 구해서 반환
SELECT MIN(EMP_NAME), MIN(SALARY), MIN(HIRE_DATE)
FROM EMPLOYEE;

--3. MAX(모든타임의 가능) : 해당 컬럼값 중 가장 큰 값을 구해서 반환
SELECT MAX(EMP_NAME), MAX(SALARY), MAX(HIRE_DATE)
FROM EMPLOYEE;

--5. COUNT(* |컬럼| DISTINCT(중복제거) 컬럼) : 해당 조건에 맞는 행의 갯구를 써서 변환
--COUNT(*) :조회된 결과에 모든 행의 갯수를 세서 변환
--COUNT(컬럼) :제시한 행당 컬럼값이 NULL이 아닌것만 행의 갯수를 세서 변환
