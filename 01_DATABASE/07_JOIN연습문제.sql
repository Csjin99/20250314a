...
WHERE EMP_NO LIKE '7%-2%' AND EMP_NAME LIKE '전%' OR EMP_NO LIKE '%-4%';
...
SELECT E.EMP_NAME, E.DEPT_CODE, E.EMP_NAME
FROM EMPLOYEE E
...
FLOOR(CEIL(MONTHS_BETWEEN (SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6), 'RR/MM/DD')))/12) AS "나이",
DEPT_TITLE, JOB_NAME
    FROM EMPLOYEE
    JOIN JOB USING (JOB_CODE)
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
