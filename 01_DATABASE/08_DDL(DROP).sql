...
---1_3) 컬럼 삭제(DROP COLUMN) : DROP COLUMN 삭제하고하는 컬럼
...
--컬럼이 하나 이하로는 갈수없어서 한개남으면 지울 수 없다.
----------------------------------------------------------------
...
/*
    ...
    -FORRIGN KEY: ALTER TABLE 테이블명 ADD FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명(컬럼명);
    -UNIQUE : ALTER TABLE 테이블명 UNIQUE(컬럼명);
    -CHECK: ALTER TABLE 테이블명 ADD CHECK(컬럼에대한 조건식);
    -NOT NULL: ALTER TABLE 테이블명 MODIFY 컬럼명 NOT NULL;
    
    제약조건명 지정하고자한다면 CONSTARINT (제약조거명) 제약조건
...
ALTER TABLE DEPT_TABLE DROP CONSTRAINT DTABLE_UQ
                       MODIFY LNAME NULL;

---------------------------------------------------------------------
--테이블 삭제
DROP TABLE DEPT_TABLE;
--어딘가에 참조되고있는 부모테이블은 함부로 삭제가 되지않는다.
--만약지우고자한다면
--1. 자식테이블먼저 전부삭제
--2. 그냥 부모테이블만 삭제하는데 제약조건까지 삭제한다.
...
ALTER TABLE DEPT_TABLE RENAME CONSTRAINT SYS_C008657 TO DTABLE_ID_NN;

--3_3)테이블명 변경 : RENAME TO 가뿔테이블명
