...
SELECT HIRE_DATE AS "고용일", EMP_NAME AS "직원명",SALARY AS "급여"
FROM EMPLOYEE;
...
(((SALARY + (SALARY * NVL(BONUS,0)))*12) - ((SALARY * 12) * 0.03)), HIRE_DATE
FROM EMPLOYEE
...
SELECT EMP_NAME, FLOOR(ABS(HIRE_DATE -SYSDATE)),
FLOOR(ABS(SYSDATE-HIRE_DATE))
FROM EMPLOYEE;
...
SUBSTR(EMP_NO,1,2) || '년' || SUBSTR(EMP_NO,3,2) || '월' ||SUBSTR(EMP_NO,4,2)|| '일' AS "생일",
...
--(단, 부서코드 오름차순으로 정렬)
SELECT EMP_NAME,DECODE(DEPT_CODE, 'D5', '총무부', 'D6', '기획구', 'D9', '영업부')
FROM EMPLOYEE
...
SELECT COUNT(*) AS "전체직원수",
   COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2001, 1, NULL)) AS "2001년 입사자",
   COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2002, 1, NULL)) AS "2002년 입사자",
   COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2003, 1, NULL)) AS "2003년 입사자",
   COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), 2004, 1, NULL)) AS "2004년 입사자"  
FROM EMPLOYEE;