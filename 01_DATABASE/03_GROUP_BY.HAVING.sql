...
--각 직급별 총 사원수, 보너스를 받은 사람 수, 급여합, 평균 급여, 최저시급, 최고 시급, 최고 급여(정렬= 직급 내림차순)
...
--GROUP BY 절에 함수식 사용가능(그룹을 나눌 수 이쓴 기준만 넣어주면된다.)
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', 'G'), COUNT(*)
FROM EMPLOYEE
...
--직급별 직급코드, 총급여합(단 총 급여합이 1000만원이상인 지급만 조회)
