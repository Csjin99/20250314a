...
public class Run {
	...
	public static void main(String[] args) {
		...
		Avante c2 = new Avante("햐얀색", "LPG",2022);
		c2.dirve(); //Avante클래스에서 오버라이딩 돈 drive() 호출
		c2.moveAvante();
		((Car)c2).drive(); //부모참조변수로 업캐스팅 가능 -> 오버라이딩된 메서드는 생성된 메서드기준으로 호출
		//((Car)c2).moveAvante(); //업캐스팅시 자식요소의 메모리 접근불가
	
		
		//3.부모타입 래퍼런스로 자식객체를 다루는 경우(업캐스팅)
		Car c3 = new Sonata("검정", "디젤",2015);
		//c3.moveSonata(); 부모타입의 참조변수이기 때문에 자식요소의 메서드 접근불가
