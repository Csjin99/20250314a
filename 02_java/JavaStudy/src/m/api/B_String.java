...
public class B_String {
	public void method01() {
		...
		//String클래스에서 toString메서드는 이미 오버라이딩 되어있다.
		
		System.out.println(str1 == str2);
		//String객체의 주소값을 비교하기 때문에 false가 나온다.
		
		System.out.println(str1.equals(str2));
		//String클래스에서 equals메서드는 이미 오버라이딩 되어있다(주소값 비교가아닌 값비교)
		
		System.out.println(str1.hashCode());
		System.out.println(str2.hashCode());
		//String클래스에서 hashcode메서드도 이미 오버라이딩이 되어있다.(주소값이 아닌 무자열을 가지고 만든다)
		...
		String str4 ="하이";
		//리터널 제시시 상수풀영역에 들어감
		//String pool특징: 동일한 문자열을 중복해서 가지지않는다.
	...
	public void method02() {
		...
		//문자열.charAt(int indes) : char
		//문자열에서 전달받은 index위치에 문자를 추출해서 반환한다
		...
		//문자열.substring(시작위치, 끝위치(생략가능))
		//문자열을 시작위치부터 끝위치-1까지 추출해서 반환
		System.out.println(str1.substring(6));
		System.out.println(str1.substring(6,9));
		
		//문자열.replace(대성문자,변경할문자) : String
		//문자열에서 대상문자를 찾아서 변경한문자로 변환된 문자열 반환
		String str4 =str1.replace('l', 'c');
		System.out.println("str1 : " + str1);
		System.out.println("str4 : " + str4);
		
		//문자열.touppercase() : string ->문자열을 전부 대문자변환
		//문자열.toLowerCase() : string ->문자열을 전부 소문자변환
		System.out.println(str1.toUpperCase());
		System.out.println(str1.toLowerCase());
		
		//문자열.trim() : String
		//문자열의 앞뒤 공백을 제거시킨 새문자열 리턴
		String str5 = "       ja          va        ";
