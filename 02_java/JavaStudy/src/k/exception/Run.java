...
public class Run {
	 ...
	 * - 논리에러 : 문법적으로 문제없고 실행했을 때도 문제가 없지만 프로그램의 의도와 맞지않는 동작이나 결과가 나타나는 에러
	 * 
	 * 컴파일에러, 런타임에러, 논리에러같은 개발자가 예측가능하고 수정할 수 있는 에러들을 가지고 작업
	 * -> 예외 -> Exception
	 * 
	 * 이런 "예외"가 발생했을 때 "처리"하는 방법을 "예외처리"라고 한다.
	 * 
	 * 예외를 처리하는 목적
	 * -예외를 처리하지않고 그대로 예외가 발생할 경우 프로그램이 비정상적으로 종료될 수 있기 떄문에 이를 방지해야한다.
	 * 
	 * 예외를 처리하느 방법
	 * 1. try-catch문을 이용
	 * 2. throws를 이용(떠넘기기)
