...
public class ListRun {
	...
	public static void main(String[] args) {
		...
		//지정된 크기보다 더 많이 넣어도 에러가 발생하지 않는다. -> 장점1. 크기제약없다.
		//다양한 타입의 데이터를 담을 수 있다. -> 장점2. 여러타입 보관가능
		
		System.out.println(list); //list의 특징 : 순서를 유지하면서 담긴다(0번인덱스부터 차근차근)

		
		//2.add(int index, E e) : 직접인덱스를 지정해서 해당위치에 데이터를 추가할 수 있다.
		...
		//7. subList9int index1, int index2) : List -> 요소들을 출력해서 새로운 List로 반환
		List sub = list.subList(1, 3);
		System.out.println(sub);
		
		//8. addAll(Collection c) : 컬렉션을 통째로 뒤에 추가할 수있음
		list.addAll(sub);
		System.out.println(list);
