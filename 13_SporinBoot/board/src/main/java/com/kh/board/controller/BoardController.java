...
public class BoardController {
    ...
    @GetMapping("/{boardId}")
    public ResponseEntity<BoardResponse.DetailDTO> getBoardDetail(@PathVariable Long boardId) {
        Board board = boardService.findById(boardId);
        if (board == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }

        BoardResponse.DetailDTO detailDTO = BoardResponse.DetailDTO.formEntity(board);
